...
if ($_SESSION['role'] !== 'dosen') {
    ...
    $projek_id = $_GET['projek_id'] ?? null; // ID projek

    // Cek apakah ID projek tersedia
    if (!$projek_id) {
        redirect_alert('warning', 'Projek tidak ditemukan!', 'projek.php');
    } else {
        ...
        if (!$projek) {
            redirect_alert('error', 'Projek tidak ditemukan.', 'projek.php');
        }
    }
}
...
<div class="container py-5 mt-5">
  ...
  <form action="" method="POST" class="border p-3 rounded shadow-sm mb-4">
    ...
    <button type="submit" name="simpan_komentar" class="btn btn-clr">
      Kirim Komentar
    </button>
  </form>
  ...
  <div class="border p-3 rounded shadow-sm mb-5">
    ...
    <?php if ($komentarResult && mysqli_num_rows($komentarResult) > 0): ?>
